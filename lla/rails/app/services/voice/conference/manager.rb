class Voice::Conference::Manager
  ...
  def join_agent!
    ...
    return unless user_id && claim_for_user!(user_id)
  ...
  def claim_for_user!(user_id)
    return false unless participant_authorized?(user_id)
  ...
  def participant_authorized?(user_id)
    membership = AccountUser.find_by(account_id: call.account_id, user_id: user_id)
    membership.present? && (membership.administrator? || call.inbox.inbox_members.exists?(user_id: user_id))
  end
