module Lla::Webhooks::WhatsappEventsJob
  def handle_message_events(channel, params)
    return track_voice_event(channel) { handle_call_events(channel, params) } if call_event?(params)
    return track_voice_event(channel) { handle_call_permission_reply(channel, params) } if call_permission_reply?(params)
  ...
  def with_call_lock(channel, call_id, &)
    sender_digest = Digest::SHA256.hexdigest(call_id.to_s)
    lock_key = format(::Redis::Alfred::WHATSAPP_MESSAGE_MUTEX,
                      inbox_id: channel.inbox.id, sender_id: "call:#{sender_digest}")
  ...
  def track_voice_event(channel)
    event = Lla::Voice::CallEvent.find_by(
      id: @lla_voice_event_id,
