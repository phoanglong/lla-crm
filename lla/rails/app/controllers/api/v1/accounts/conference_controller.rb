class Api::V1::Accounts::ConferenceController < Api::V1::Accounts::BaseController
  ...
  def set_voice_inbox_for_conference
    ...
    voice_enabled = @voice_inbox.channel.is_a?(Channel::TwilioSms) && @voice_inbox.channel.voice_enabled?
    raise ActiveRecord::RecordNotFound, 'Voice not enabled' unless voice_enabled
  end
  ...
  def finalize_as_agent_reject!(call)
    # Call#transition_to! re-checks under its own row lock, so a concurrent
    # provider callback cannot be rolled back to rejected.
    rejected = call.transition_to!(
      'rejected', end_reason: 'agent_rejected', accepted_by_agent: Current.user, from_status: 'ringing'
    ) == :applied
