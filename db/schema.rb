...
ActiveRecord::Schema[7.1].define(version: 2026_08_17_180000) do
  ...
  create_table "lla_custom_domain_operations", force: :cascade do |t|
    ...
    t.check_constraint "operation_type::text = ANY (ARRAY['provision'::character varying, 'verify'::character varying, 'reverify'::character varying, 'remove'::character varying, 'reconcile'::character varying]::text[])", name: "chk_lla_custom_domain_ops_type"
  ...
  create_table "lla_custom_domain_tombstones", force: :cascade do |t|
    ...
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
    t.index ["account_id", "hostname"], name: "idx_lla_custom_domain_tombstones_host", unique: true
    t.index ["state", "created_at"], name: "idx_lla_custom_domain_tombstones_state"
    t.check_constraint "char_length(hostname::text) >= 1 AND char_length(hostname::text) <= 253 AND hostname::text !~ '[[:space:][:cntrl:]]'::text", name: "chk_lla_custom_domain_tombstones_hostname"
