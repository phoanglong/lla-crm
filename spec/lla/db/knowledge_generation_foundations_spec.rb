...
RSpec.describe 'LLA knowledge generation database foundations', type: :model do
  ...
  it 'persists tenant-bound operation, item, and encrypted outbox structures' do
    ...
    aggregate_failures do
      expect(operation_columns).to include('account_id', 'portal_id', 'user_id', 'idempotency_digest', 'consent_digest')
  ...
  it 'enforces digest, counter, state, and retry bounds' do
    ...
    aggregate_failures do
      ...
      expect(outbox_checks).to include('chk_lla_knowledge_outboxes_digests', 'chk_lla_knowledge_outboxes_attempts')
    end
  end
end
