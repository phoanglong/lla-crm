...
describe Whatsapp::Providers::WhatsappCloudService do
  ...
  describe '#update_calling_status' do
    it 'rejects an unknown status before making a provider request' do
      expect { service.update_calling_status('UNKNOWN') }
        .to raise_error(ArgumentError, 'Invalid WhatsApp calling status')

      expect(a_request(:post, %r{/settings\z})).not_to have_been_made
    end
  end
  ...
  describe '#initiate_call' do
    ...
    it 'raises Voice::CallErrors::NoCallPermission when Meta returns error code 138006' do
      ...
      expect { service.initiate_call('15551234567', sdp_offer) }
        .to raise_error(Voice::CallErrors::NoCallPermission, 'WhatsApp call permission is required')
    end
    ...
    it 'raises Voice::CallErrors::CallFailed with a fallback message when the error body is non-JSON' do
      ...
      expect { service.initiate_call('15551234567', sdp_offer) }
        .to raise_error(Voice::CallErrors::CallFailed, 'WhatsApp call provider request failed')
    end
  end
end
