...
describe Voice::Provider::Twilio::TokenService do
  ...
  it 'returns a token payload with expected keys' do
    ...
    expect(payload).not_to include(:account_sid, :phone_number, :twiml_endpoint)
    expect(Twilio::JWT::AccessToken).to have_received(:new).with(
      voice_channel.account_sid,
      voice_channel.api_key_sid,
      voice_channel.api_key_secret,
      hash_including(ttl: 5.minutes.to_i)
    )
  end

  it 'rejects an agent without access to the inbox' do
    unauthorized_user = create(:user, account: account)

    expect { described_class.new(inbox: inbox, user: unauthorized_user, account: account).generate }
