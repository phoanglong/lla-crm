...
describe Voice::Provider::Twilio::Adapter do
  ...
  it 'initiates an outbound call with expected params' do
    ...
    allow(channel).to receive(:client).and_return(client_double)
  ...
  it 'rejects a non-E.164 destination before calling Twilio' do
    allow(channel).to receive(:client).and_return(client_double)
    allow(calls_double).to receive(:create)

    expect { adapter.initiate_call(to: '001-555-000-1111') }
      .to raise_error(ArgumentError, 'Destination must be an E.164 phone number')
    expect(calls_double).not_to have_received(:create)
  end
end
