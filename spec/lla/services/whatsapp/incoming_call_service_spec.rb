...
describe Whatsapp::IncomingCallService do
  ...
  describe 'inbound connect' do
    ...
    it 'creates the Call + Conversation + voice_call message and broadcasts voice_call.incoming' do
      ...
      expect(call.meta['sdp_offer']).to be_nil
      expect(Lla::Voice::SdpStore.read(call: call, kind: 'offer')).to eq(sdp_offer)
  ...
  describe 'outbound connect (existing call)' do
    ...
    let!(:call) do
      ...
      create(:call, account: account, inbox: inbox, conversation: conversation, contact: conversation.contact,
                    provider: :whatsapp, direction: :outgoing, status: 'ringing', accepted_by_agent: agent,
                    provider_call_id: provider_call_id)
    end
    ...
    it 'stores the SDP answer and broadcasts voice_call.outbound_connected without flipping to in_progress' do
      ...
      expect(Lla::Voice::SdpStore.read(call: call, kind: 'answer')).to include('a=setup:active')
      expect(ActionCable.server).to have_received(:broadcast).with(
        agent.pubsub_token,
  ...
  describe 'terminate' do
    ...
    let!(:call) do
      conversation = create(:conversation, account: account, inbox: inbox, assignee: agent)
      create(:call, account: account, inbox: inbox, conversation: conversation, contact: conversation.contact,
                    provider: :whatsapp, direction: :incoming, status: 'in_progress', accepted_by_agent: agent,
                    provider_call_id: provider_call_id)
    end
  ...
  describe 'terminate with no local row yet' do
    ...
    after { Redis::Alfred.delete(tombstone_key(tombstone_call_id)) }

    def tombstone_key(call_id)
      digest = Digest::SHA256.hexdigest(call_id)
      "LLA_WHATSAPP_CALL_TERMINATE::#{account.id}:#{inbox.id}:#{digest}"
    end
    ...
    it 'tombstones the terminate instead of materialising an inbound missed-call row' do
      ...
      expect { described_class.new(inbox: inbox, params: params).perform }
        .not_to change(Call, :count)
      expect(Redis::Alfred.get(tombstone_key(tombstone_call_id))).to be_present
      expect(ActionCable.server).not_to have_received(:broadcast)
    end
  ...
  describe 'outbound connect with no local row yet' do
    it 'does not mint an inbound call when sdp_type is answer' do
      ...
      expect { described_class.new(inbox: inbox, params: params).perform }
        .not_to change(Call, :count)
      expect(Rails.logger).to have_received(:warn).with(/LLA_WHATSAPP_OUTBOUND_CONNECT_UNKNOWN/)
      expect(ActionCable.server).not_to have_received(:broadcast)
    end
  end
  ...
  describe 'duplicate inbound connect' do
    ...
    it 'logs and ignores rather than treating it as outbound' do
      ...
      expect(Rails.logger).to have_received(:info).with(/LLA_WHATSAPP_INBOUND_CONNECT_DUPLICATE/)
      expect(ActionCable.server).not_to have_received(:broadcast)
    end
  end
  ...
  describe 'unknown event' do
    it 'logs a warning and does not raise' do
      ...
      expect(Rails.logger).to have_received(:warn).with(/LLA_WHATSAPP_CALL_EVENT_IGNORED/)
    end
  end
  ...
  describe 'multiple calls in one webhook payload' do
    it 'processes every call in the array' do
      ...
      params = {
        calls: [
          { id: 'wacid_a', from: from_number, event: 'connect', session: { sdp: 'v=0 sdp_a', sdp_type: 'offer' } },
          { id: 'wacid_b', from: '15550002222', event: 'connect', session: { sdp: 'v=0 sdp_b', sdp_type: 'offer' } }
        ]
      }
