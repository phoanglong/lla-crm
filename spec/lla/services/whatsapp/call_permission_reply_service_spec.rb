...
describe Whatsapp::CallPermissionReplyService do
  ...
  let!(:conversation) do
    create(:conversation, account: account, inbox: inbox, contact: contact, contact_inbox: contact_inbox,
                          assignee: agent, status: :open,
                          additional_attributes: {
                            'call_permission_requested_at' => Time.current.iso8601,
                            'call_permission_request_message_id_digest' => Digest::SHA256.hexdigest(request_wamid)
                          })
  end
  ...
  def reply_params(response:, context_id: request_wamid, from: '15550001111')
    interactive = { type: 'call_permission_reply',
                    call_permission_reply: { response: response, is_permanent: false } }
    message = { from: from, type: 'interactive', interactive: interactive }
  ...
  it 'clears both permission flags and broadcasts voice_call.permission_granted on accept' do
    ...
    expect(attrs).not_to include('call_permission_request_message_id_digest')
  ...
  it 'matches the originating conversation by context.id when the contact has multiple pending requests' do
    ...
    other_open = create(:conversation, account: account, inbox: inbox, contact: contact, contact_inbox: contact_inbox,
                                       ...
                                       additional_attributes: {
                                         'call_permission_requested_at' => Time.current.iso8601,
                                         'call_permission_request_message_id_digest' => Digest::SHA256.hexdigest(other_request_wamid)
                                       })
    allow(ActionCable.server).to receive(:broadcast)
    ...
    expect(other_open.reload.additional_attributes).not_to include('call_permission_request_message_id_digest')
    expect(conversation.reload.additional_attributes).to include('call_permission_request_message_id_digest')
    expect(ActionCable.server).to have_received(:broadcast).with(
      agent.pubsub_token,
  ...
  it 'is a no-op when the reply sender does not own the requesting conversation' do
    allow(ActionCable.server).to receive(:broadcast)

    described_class.new(
      inbox: inbox,
      params: reply_params(response: 'accept', from: '15559999999')
    ).perform
