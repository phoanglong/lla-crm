...
RSpec.describe Api::V1::Accounts::ConferenceController, type: :request do
  ...
  describe 'DELETE /conference' do
    ...
    context 'when authenticated agent with inbox access' do
      ...
      it 'does not overwrite a provider acceptance that races the agent rejection' do
        allow(conference_service).to receive(:end_conference) do
          Call.find_by(provider_call_id: 'CALL123').transition_to!('in_progress')
        end
        ...
        expect(response).to have_http_status(:ok)
        expect(Call.find_by(provider_call_id: 'CALL123').status).to eq('in_progress')
      end
