...
RSpec.describe 'WhatsApp Calls API', type: :request do
  ...
  let(:sdp_answer) { "v=0\r\no=lla 2 2 IN IP4 127.0.0.1\r\n" }
  let(:initiate_headers) { agent.create_new_auth_token.merge('Idempotency-Key' => 'spec-call-request-001') }
  ...
  describe 'POST /api/v1/accounts/:account_id/whatsapp_calls/initiate' do
    ...
    it 'sends a permission request and records the wamid when Meta returns NoCallPermission' do
      ...
      expect(attrs['call_permission_request_message_id_digest']).to eq(Digest::SHA256.hexdigest('wamid.req_xyz'))
    end
    ...
    it 'returns 422 without dialing when Idempotency-Key is missing' do
      ...
      expect(response).to have_http_status(:unprocessable_entity)
      expect(response.parsed_body['error']).to eq('Idempotency-Key required')
  ...
  describe 'POST /api/v1/accounts/:account_id/whatsapp_calls/:id/upload_recording' do
    before do
      ...
      call.update!(message_id: message.id, meta: call.meta.merge('recording_consent_id' => 'consent-spec-1'))
