...
RSpec.describe 'LLA session audit API', type: :request do
  let!(:account) { create(:account) }
  let!(:user) { create(:user, password: 'Password1!', account: account) }

  describe 'POST /sign_in' do
    context 'with a SAML user attempting password login' do
      # Chỉ kiểm tra chốt chặn mật khẩu — phần cấu hình SAML (AccountSamlSettings,
      # omniauth, luồng sso_auth_token) thuộc Wave B3.
      ...
      it 'prevents login and returns SAML authentication error' do
        ...
        end.not_to change(Lla::AuditLog, :count)
      ...
      it 'creates a sign_in audit event wwith valid credentials' do
        ...
        expect do
          post new_user_session_url,
               params: params,
               as: :json
        end.to change(Lla::AuditLog, :count).by(1)
